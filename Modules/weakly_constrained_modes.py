...
def calculate_modes(real_mask, fourier_mask, reality_constraint=False,
    ...
    real_mask_shifted = numpy.fft.fftshift(real_mask)
    fourier_mask_shifted = numpy.fft.fftshift(fourier_mask)

    if reality_constraint:
        sub_matrix = dft.dft_2d_masked_real(real_mask.shape[0],
                                            real_mask.shape[1],
                                            real_mask_shifted,
                                            fourier_mask_shifted)
    else:
        sub_matrix = dft.dft_2d_masked(real_mask.shape[0],
                                       real_mask.shape[1],
                                       real_mask_shifted,
                                       fourier_mask_shifted)

    real_modes_raw, singular_values, fourier_modes_raw1 = numpy.linalg.linalg.svd(sub_matrix, full_matrices=False)
    fourier_modes_raw.dtype = "complex128"
    inverse_singular_values = numpy.sqrt(1.-singular_values**2)

    if not number_of_modes:
        number_of_modes = len(singular_values)

    real_modes_shifted = numpy.zeros((number_of_modes,
                                      real_mask_shifted.shape[0],
                                      real_mask_shifted.shape[1]),
                                     dtype="complex128")
    real_modes_shifted[:, real_mask_shifted] = real_modes_raw.T[:number_of_modes]
    real_modes = numpy.fft.fftshift(real_modes_shifted, axes=(1, 2))
    fourier_modes_shifted = numpy.zeros((number_of_modes,
    ...
    fourier_modes = numpy.fft.fftshift(fourier_modes_shifted, axes=(1, 2))
