...
thresholds = pylab.array([0.25,0.4,0.55,0.7,0.75])*max(density_map.flatten())
out_files = ['isosurface_25.data','isosurface_40.data','isosurface_55.data','isosurface_70.data','isosurface_75.data']

for threshold,out_file in zip(thresholds,out_files):
    print 'threshold = %g' % threshold
    print 'filename = %s' % out_file
    #out_file = 'isosurface_75.data'
    ...
    for x in range(pylab.shape(density_map)[0]-1):
        if x % int((pylab.shape(density_map)[0]-1)*0.1) == 0:
            print "%g percent done" % (float(x)/float(pylab.shape(density_map)[0]-1))
        for y in range(pylab.shape(density_map)[1]-1):
            for z in range(pylab.shape(density_map)[2]-1):
                c = (corners[x:x+2,y:y+2,z:z+2],(x,y,z),density_map[x:x+2,y:y+2,z:z+2])
    ...
    center = pylab.array(pylab.shape(density_map))/2.0
    scaling = 1.0/pylab.average(pylab.array(pylab.shape(density_map)))

    for i in range(len(vertices)):
        vertices[i] = tuple(scaling*(pylab.array(vertices[i])-center))
