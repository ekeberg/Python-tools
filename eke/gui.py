"""Small GUI tools for use in smaller python scripts. I think the
implementation of the Manipulator looks weirt, but it works. Needs non
...
class Manipulator(QtWidgets.QMainWindow):
    ...
    def __init__(self, function, value_ranges, value_names, parent=None):
        ...
        if (
                self._value_names and
                len(self._value_names) != self._number_of_variables
        ):
    ...
    def _create_window(self):
        ...
        for variable in range(self._number_of_variables):
            ...
            self._manipulate_functions.append(
                self._VariableChanger(self._function, variable, self._values)
            )
