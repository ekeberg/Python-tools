...
class Resampler(object):
    ...
    def _set_input_points(self, grid):
        ...
        if (len(self._input_points.shape) != 2 or self._input_points.shape[1] != 3):
            raise ValueError("Grid must be (n, 3) dimensional, {0} received.".format(
                str(self._input_points.shape)))
        self._calculate_table()
    ...
    def remap_in_place(self, input_values, output_values):
        if len(input_values) != len(self._input_points):
            raise ValueError("Array size ({0}) different thatn precalculated ({1})".format(
                len(input_values), len(self._input_points)))
        output_values[:] = 0.
...
def listdir_ext(path, ext):
    "Return all files in path with given extension. The return uses the full path"
    import os
    import re
...
def radial_distance(shape):
    ...
    for i in range(len(shape)):
        radius = (radius +
                  (axis_values[-(1+i)][(slice(0, None), ) + (_numpy.newaxis, )*i])**2)
...
def radial_average_simple(image, mask=None):
    """Calculates the radial average from the center to the
    edge (corners are not included)"""
...
def pad_with_zeros(small_image, shape):
    ...
    if len(shape) != len(small_image.shape):
        raise ValueError("Input image is {0} dimensional and size is {1} dimensional".format(
            len(small_image.shape), len(shape)))
