...
class Quaternion(object):
    ...
    def __init__(self, elements):
        if len(elements) != 4:
            raise ValueError("Quaternion must be initialzied by a lenght 4 array.")
    ...
    def __str__(self):
        return "Quaternion({0}, {1}, {2}, {3})".format(self._elements[0], self._elements[1], self._elements[2], self._elements[3])
    __repr__ = __str__

    def __getitem__(self, index):
        if not isinstance(index, slice) and (index < 0 or index >= 4):
            raise IndexError("Index must be 0, 1, 2 or 3. Not {0}".format(index))
...
def from_angle_and_dir(angle, direction):
    ...
    return Quaternion((_numpy.cos(angle/2.),
                       direction[0]/_numpy.linalg.norm(direction)*_numpy.sin(angle/2.),
                       direction[1]/_numpy.linalg.norm(direction)*_numpy.sin(angle/2.),
                       direction[2]/_numpy.linalg.norm(direction)*_numpy.sin(angle/2.)))

def rotation_matrix(quaternion):
    """Get the rotation matrix corresponding to a quaternion"""
    return _numpy.matrix([[quaternion[0]**2 + quaternion[1]**2 - quaternion[2]**2 - quaternion[3]**2,
                           2.0*quaternion[1]*quaternion[2] - 2.0*quaternion[0]*quaternion[3],
                           2.0*quaternion[1]*quaternion[3] + 2.0*quaternion[0]*quaternion[2],],
                          [2.0*quaternion[1]*quaternion[2] + 2.0*quaternion[0]*quaternion[3],
                           quaternion[0]**2 - quaternion[1]**2 + quaternion[2]**2 - quaternion[3]**2,
                           2.0*quaternion[2]*quaternion[3] - 2.0*quaternion[0]*quaternion[1]],
                          [2.0*quaternion[1]*quaternion[3] - 2.0*quaternion[0]*quaternion[2],
                           2.0*quaternion[2]*quaternion[3] + 2.0*quaternion[0]*quaternion[1],
                           quaternion[0]**2 - quaternion[1]**2 - quaternion[2]**2 + quaternion[3]**2]])
...
def average(quaternion_list):
    ...
    return Quaternion(_numpy.array([this_quaternion[:] for this_quaternion in quaternion_list]).mean(axis=0))
