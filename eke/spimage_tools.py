"""Collection of tools to make it easier to work with the spimage
python interface."""
...
def get_basic_phaser(amplitudes, support, mask=None, algorithm=None):
    ...
    if algorithm is None:
        ...
        phase_alg = _spimage.sp_phasing_hio_alloc(
            beta, _spimage.SpNoConstraints)
    else:
        phase_alg = algorithm
...
def smap(values):
    """Create spimage smap object from iterable of (iteration, value)
    pairs."""
    import collections
...
def get_constraints(real=False, positive=False):
    """It is easy to forget the name of the constraints variable. This
    function returns the appropriate one.
    """
...
def support_area(area, blur_radius):
    ...
    algorithm = _spimage.sp_support_array_init(
        _spimage.sp_support_area_alloc(blur_radius_sp, area_sp), 20)
    return algorithm
...
def center_image_2d(img, radius):
    ...
    sigma = radius
    x_coordinates = (_numpy.arange(_numpy.shape(img.image)[0], dtype='float64')
                     - _numpy.shape(img.image)[0] / 2
                     + 0.5)
    y_coordinates = (_numpy.arange(_numpy.shape(img.image)[1], dtype='float64')
                     - _numpy.shape(img.image)[1] / 2
                     + 0.5)
    ...
    _spimage.sp_image_translate(img, -(-min_x + _numpy.shape(img_bt)[0]//2),
                                -(-min_y + _numpy.shape(img_bt)[1]//2),
                                0, _spimage.SP_TRANSLATE_WRAP_AROUND)
...
def center_image_3d(img, radius):
    ...
    y_coordinates = (_numpy.arange(_numpy.shape(img.image)[1], dtype='float64')
                     - _numpy.shape(img.image)[1] / 2
                     + 0.5)
    z_coordinates = (_numpy.arange(_numpy.shape(img.image)[2], dtype='float64')
                     - _numpy.shape(img.image)[2] / 2
                     + 0.5)
    kernel = _numpy.exp(-(x_coordinates[:, _numpy.newaxis, _numpy.newaxis]**2 +
    ...
    for x_index in range(_numpy.shape(img_bt)[0]):
        for y_index in range(_numpy.shape(img_bt)[1]):
            for z_index in range(_numpy.shape(img_bt)[2]):
    ...
    _spimage.sp_image_translate(img, -(-min_z + _numpy.shape(img_bt)[0]//2),
                                -(-min_y + _numpy.shape(img_bt)[1]//2),
                                -(-min_x + _numpy.shape(img_bt)[2]//2), 1)
    shift = img
