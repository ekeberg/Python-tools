"""Calls the repeat reconstruction script to do many
reconstructions. This is obsolete for use in Uppsala since multiple
reconstructions are preferably run on the GPU cluster.
"""
import os as _os
import re as _re
...
class Reconstructer(object):
    """Calls the repeat reconstruction script to do many
    reconstructions. Made to be used together with the
    MultipleReconstructions class. Takes the configuration file to
    use, the intensities file, a directory in which to put the result,
    the number of reconstructions to perform and a dictionary
    containing changes to make to the conf file (as "name" :
    "value").
    """
...
class MultipleReconstructions(object):
    """Performs multiple reconstructions of multiple files using the
    Reconstructer class."""
    ...
    def get_file_list(self):
        ...
        files = ['%s/%s' % (self._ints_dir, f) for f in files
                 if _re.search(r'\.h5$', f)]
        files.sort()
        return files
    ...
    def start(self):
        ...
        for file_name in self._files:
            ...
            Reconstructer(self._conf_file, file_name, self._base_dir,
                          self._n_recs, self._conf_dict).start()
...
if __name__ == "__main__":
    ...
    MultipleReconstructions('/data/LCLS2011/r0138/all/uwrapc.conf',
                            '/home/ekeberg/Work/python/mimi_finder/preprocess/'
                            'processed_for_reconstruction',
