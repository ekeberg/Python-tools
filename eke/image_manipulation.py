"""A collection of image manipulation functions. Some of them uses
spimage objects and some numpy arrays.
...
def scale_image_3d(image, factor):
    ...
    image_scaled = abs(_fft.ifftn(_fft.fftshift(image_ft),
                                  [size_x, size_y, size_z]))
    return image_scaled


def crop_and_pad(image, center, side):
    """Crops the image around the center to the side given. If the cropped
    area is larger than the original image it is padded with zeros

    """
    dims = len(image.shape)
    if dims != 3 and dims != 2:
        raise ValueError("crop_and_pad: Input image must be 2 or three "
                         "dimensional")
    if len(center) != dims:
        raise ValueError("crop_and_pad: Center must be same length as image "
                         f"dimensions ({len(center)} != {dims})")
    ...
    if dims == 2:
        out_slice = (slice(low_out[0], high_out[0]),
                     slice(low_out[1], high_out[1]))
        in_slice = (slice(low_in[0], high_in[0]),
                    slice(low_in[1], high_in[1]))
    else:
        out_slice = (slice(low_out[0], high_out[0]),
                     slice(low_out[1], high_out[1]),
                     slice(low_out[2], high_out[2]))
        in_slice = (slice(low_in[0], high_in[0]),
                    slice(low_in[1], high_in[1]),
                    slice(low_in[2], high_in[2]))
    ret[out_slice] = image[in_slice]
    return ret
