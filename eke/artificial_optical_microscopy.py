...
def dic(image, dic_angle, dic_phase, dic_split):
    ...
    x = _numpy.fft.fftshift((_numpy.arange(image.shape[0])-image.shape[0]/2.+0.5)/image.shape[0])[_numpy.newaxis, :]
    y = _numpy.fft.fftshift((_numpy.arange(image.shape[1])-image.shape[1]/2.+0.5)/image.shape[1])[:, _numpy.newaxis]
