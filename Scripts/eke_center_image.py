...
def center_image(filename, outfile, sigma=3):
    ...
    side = img.image.shape[0]
    x = numpy.arange(img.image.shape[0], dtype='float64') - img.image.shape[0]/2. + 0.5
    y = numpy.arange(img.image.shape[1], dtype='float64') - img.image.shape[1]/2. + 0.5
    z = numpy.arange(img.image.shape[2], dtype='float64') - img.image.shape[2]/2. + 0.5
    ...
    spimage.sp_image_translate(img,-(-min_z + bt.shape[0]//2),
                               -(-min_y + bt.shape[1]//2),
                               -(-min_x + bt.shape[2]//2), SP_TRANSLATE_WRAP_AROUND)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--sigma", type=float, default=3,
                        help="Width of the particle that is centered in pixels (optional, defaults to 3 pixels)")
