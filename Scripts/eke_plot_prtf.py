...
class Setup:
    ...
    def rescale(self,value):
        if self.number_of_images:
            return (value-1./numpy.sqrt(float(self.number_of_images)))/(1.-1./numpy.sqrt(float(self.number_of_images)))
        else:
            return value
...
def plot_prtf(filename,setup = 0):
    ...
    for i in range(len(data[1,:])):
        if setup.rescale(data[1,i]) < numpy.exp(-1):
            break
    ax.plot([setup.convert(data[0,i]),setup.convert(data[0,i])],[0,numpy.exp(-1)])
    ax.set_ylim([0.0,1.0])

    if setup.known:
        #numpy.text(setup.convert(data[0,-1]),0.95,
        ax.text(0.95*ax.get_xlim()[1],0.95*ax.get_ylim()[1],
                   r'$R_{\mathrm{crystall}} = %g nm \quad R_{\mathrm{optical}} = %g nm$' % (1./setup.convert(data[0,i]),
                                                                                            .5/setup.convert(data[0,i])),
...
if __name__ == "__main__":
    ...
    res.add_argument("-w", "--wavelength", type=float, help="Wavlength used in the experiment [nm]")
